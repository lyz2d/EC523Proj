...
class SIFT_ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels = 3, dim_head = 64):
        ...
        self.laf_predictor = LafPredictor(patch_count=14)
        self.laf_patch_extractor = LafPatchExtractor(patch_size=patch_size)

    def forward(self, img):
        device = img.device

        # x is traditional patches. It will be used to predict the laf.
        x = self.to_patch_embedding(img)
